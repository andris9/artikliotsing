...
var rewriteUrlRules = [
    ...
    {
        "from": /^(http:\/\/(?:[^.]+\.)?(?:tallinna|tartu)?(?:postimees|tarbija24|e24|elu24|ilmajaam|juhtimine|naine24)\.ee\/(\d+))\/\b/i,
        "to": "$1/print/"
    },
    {
...
function fetchArticle(articleData){
    ...
    fetch.fetchUrl(articleUrl, {timeout: 30 * 1000}, function(err, meta, body){
        var data;
...
function storeArticle(articleData){
    fetch.fetchUrl(config.elasticsearch.replace(/ARTICLE_ID/, crypto.createHash("sha1").update(articleData.url).digest("hex")), {method:"PUT", payload: JSON.stringify(articleData)}, function(err, meta, body){
        if(err){
